...
class hardware_exp:
    def __init__(
        ...
        self.hw_link_dir = f"{sc['secda_init_path']}/{sc['hw_link_dir']}/"
        ...
        acc_link_folder = config["acc_link_folder"]
        ...
        self.hlx_script = (
            f"{sc['secda_init_path']}/{sc['hlx_scripts']}/{config['hlx_tcl_script']}"
        )
    ...
    def create_run_script(self, output_path):
        ...
        run_dict = {
            ...
            "board": self.board,
            "exp_name": self.config.get("del", ""),
            "exp_version": self.config.get("del_version", ""),
...
def process_hw_config(hw_config_file):
    ...
    if not os.path.exists(hw_config_file):
        hw_config_file = find_hw_config(
            f"{sc['secda_init_path']}/{sc['hw_configs']}",
            hw_config_file.replace(".json", ""),
        )
    hw_config = load_config(hw_config_file)

    # Creates the necessary directories
    out_dir = os.environ.get("SECDA_OUT_DIR", sc["out_dir"])
    out_dir = os.path.abspath(out_dir)
    hw_link_dir = f"{sc['secda_init_path']}/{sc['hw_link_dir']}/"
    acc_link_folder = hw_config["acc_link_folder"]
    if os.path.isabs(acc_link_folder):
        acc_link_dir = os.path.abspath(acc_link_folder)
    else:
        acc_link_dir = os.path.abspath(hw_link_dir + acc_link_folder)
    if not os.path.exists(acc_link_dir):
        os.makedirs(acc_link_dir)
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)

    # Symlinks the accelerator source files to the hardware link directory
    acc_src = hw_config["acc_src"]
    ...
    for file in os.listdir(acc_src_dir):
        if file.endswith(".cc") or file.endswith(".h"):
            ...
            target = os.path.abspath(acc_link_dir + "/")

...
BITSTREAM_NAME="&{bitstream}${BITSTREAM_SUFFIX}"
...
if [ $run_hls == 1 ]; then 
  ...
  if [ "&{board}" = "KRIA" ] ; then
    echo "Updating Floating point IPs for KV260"
...
if [ $copy_bits == 1 ]; then 
  ...
  cp ./generated_files/&{acc_tag}.hwh ../../../hardware_automation/bitstreams/&{board}/${BITSTREAM_NAME}.hwh
  cp ./generated_files/&{acc_tag}.bit ../../../hardware_automation/bitstreams/&{board}/${BITSTREAM_NAME}.bit
 ...
 # check board is connected to the network
  ...
  if [ $board_connected == 1 ]; then
    ...
    exp_key="&{exp_name}"
    if [ -n "&{exp_version}" ]; then exp_key="${exp_key}_v&{exp_version}"; fi
    board_exp_dir="$(dirname "&{board_dir}")/experiments/${exp_key}"
    board_bits_dir="$board_exp_dir/bitstreams"
    ssh -q -t -p &{board_port} &{board_user}@&{board_hostname} "mkdir -p $board_bits_dir"
    rsync -r -av -e 'ssh -p &{board_port}' ./generated_files/&{acc_tag}.bit &{board_user}@&{board_hostname}:$board_bits_dir/${BITSTREAM_NAME}.bit
    rsync -r -av -e 'ssh -p &{board_port}' ./generated_files/&{acc_tag}.hwh &{board_user}@&{board_hostname}:$board_bits_dir/${BITSTREAM_NAME}.hwh
  else
